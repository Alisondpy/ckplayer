...
function TouristTips(options){
    var _this = this;
    var defaults = {selector:'#jModeVideoBox'}
...
TouristTips.prototype._initEvent = function() {
    ...
    _this.login.on('click', function() {
     window.location.href = 'https://www.baidu.com';//登录操作
    });
}
...
login.prototype.gitNick = function(){
    return nick;
};
...
// 封装play.js方法
    ...
    function Player(selector, options) {
        ...
        var defaults = {
            ...
            swfPlayer: './ckplayer/6.8.0/ckplayer.swf' || '', //必填
            embed: {
                width: '100%',
                height: '500'
            },
            ...
            flash: {
                f: './ckplayer/ckplayer6.7/m3u8.swf' || '', //必填 要播放文件的路径
                a: 'http://n03.wbc.zhongzhihui.com/cldata/youyong/courseware/10091/2016/12/2016-12-16/515921b34a3d0c9d195a7249e9c12bbf/t/playlist.m3u8' || '', //必填 如果要调用m3u8格式的文件，必须要用到的播放插件【调用时的参数，只有当s>0的时候有效】
    ...
    //继承自定义事件
    inherits(Player, window.EventEmitter);
